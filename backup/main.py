...
from psychopy_process         import start_psychopy, stop_psychopy, timeout_value
...
def wait_psychopy_stop(
    ...
    state = marker_state if marker_state is not None else {}
    ...
    while not stop_event.is_set():
        ...
        if streams:
            ...
            state["stream_connected_at"] = time.monotonic()
            break
    ...
    while not stop_event.is_set():
        ...
        now = time.monotonic()
        state.setdefault("first_marker_at", now)
        state["last_marker_at"] = now
        state["last_marker"] = sample[0]

        if marker_is_stop(sample[0], targets):
            state["block_end_received"] = True
...
def run_block(cfg: AppConfig, raw: dict, label: str, mode: str, decoder: bool = False, model_prefix: str | None = None) -> dict:
    ...
    # O PsychoPy é iniciado somente depois que aquisição/logger/watchers já estão de pé.
    try:
        psychopy_proc = start_psychopy(
            cfg, raw, project_dir=Path(__file__).resolve().parent
        )
    ...
    startup_timeout = timeout_value(raw, "startup_timeout_s", 60.0)
    ...
    try:
        while not stop_event.wait(0.2):
            ...
            if psychopy_proc is not None:
                ...
                if rc is not None:
                    # Pequena tolerância: o processo pode fechar logo após emitir BLOCK_END,
                    # enquanto o inlet LSL ainda está entregando a última amostra.
                ...
                if startup_timeout is not None and first_marker is None and psychopy_started_mono is not None:
                    if now - psychopy_started_mono > startup_timeout:
                        print(
                            f"[main] PsychoPy não produziu nenhum marcador em "
                            f"{startup_timeout:.1f}s; encerrando bloco."
                        )
                ...
                if stall_timeout is not None and last_marker is not None:
                    if now - last_marker > stall_timeout:
                        print(
                            f"[main] Nenhum marcador PsychoPy há {stall_timeout:.1f}s. "
                            "Assumindo travamento."
                        )
        ...
        # Primeiro sinalizamos todas as threads; depois damos ao PsychoPy a chance
        # de salvar/fechar normalmente. Se ele travou, stop_psychopy força o término.
        stop_event.set()
    ...
    if not psychopy_failed and (
        marker_state.get("block_end_received") or stop_reason == "block_end_or_external_stop"
    ):
        stop_reason = "block_end"

    ended_at = time.time()
    ok = not psychopy_failed and stop_reason not in {
        "psychopy_start_failed",
        "psychopy_exited_early",
        "psychopy_startup_timeout",
        "psychopy_marker_stall",
        "block_timeout",
        "keyboard_interrupt",
    }
...
def run_training_stage(cfg: AppConfig, raw: dict, label: str, session_type: str) -> str:
    ...
    while True:
        ...
        if not info.get("ok", True):
            print(
                f"[main] Bloco incompleto ({info.get('stop_reason')}); "
                "não vou treinar o classificador com estes dados."
            )
            action = ask_choice(
                "O que fazer agora?",
                {"r": "refazer em um novo bloco", "f": "finalizar a sessão"},
                default="r",
            )
            if action == "r":
                block_n += 1
...
def run_online_stage(cfg: AppConfig, raw: dict, online_type: str, model_prefix: str) -> str:
    ...
    while True:
        ...
        try:
            info = run_block(cfg_online, raw, label=f"Online {block_n}", mode="realtime", decoder=True, model_prefix=model_prefix)
        finally:
            stop_realtime_plot(plot_proc)

        if not info.get("ok", True):
            print(f"[main] Online incompleto: {info.get('stop_reason')}.")
