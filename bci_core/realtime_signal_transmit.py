...
Apenas publica o sinal. A gravação oficial de EEG + marcadores é responsabilidade
de receive_data_log.py, igual ao fluxo com g.HIamp.
"""
import threading
...
def find_markers(cfg: AppConfig, stop_event: threading.Event | None = None) -> StreamInlet | None:
...
def marker_thread(cfg: AppConfig, inlet, q, stop_event: threading.Event):
    last = None
    wave = burst(cfg)
    codes = cfg.codes
...
def run_transmission(cfg: AppConfig, mode: str = "train", stop_event: threading.Event | None = None):
    ...
    inlet    = find_markers(cfg, stop_event=stop_event)
    if inlet is None:
        print("[sim] Stop recebido antes de encontrar o stream de marcadores.")
        return
    q = deque()

    th = threading.Thread(
        target=marker_thread,
        args=(cfg, inlet, q, stop_event),
        daemon=True,
    )
    th.start()

    try:
        stream(cfg, outlet, q, stop_event)
    finally:
        th.join(timeout=1.0)
