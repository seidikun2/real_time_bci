...
def _run_id_from_signal(path: str) -> str:
    import re
    m = re.search(r"_(\d{8}_\d{6})\.csv$", os.path.basename(path))
...
def run_check_data(cfg: AppConfig, mode: str = "train", markers_file: Optional[str] = None, signal_file: Optional[str] = None, save_png: Optional[bool] = None):
    ...
    fig2.suptitle("Janelas usadas pelo modelo — mesmo filtro do decoder", y=0.995)
    plt.tight_layout()

    if save_png:
        # QC é técnico e fica fora da vista principal dos CSVs/mapa.
        qc_dir = os.path.join(os.path.dirname(os.path.abspath(sig_path)), "_qc", _run_id_from_signal(sig_path))
        os.makedirs(qc_dir, exist_ok=True)
        p1 = os.path.join(qc_dir, "stack_raw_markers.png")
        p2 = os.path.join(qc_dir, "model_windows.png")
