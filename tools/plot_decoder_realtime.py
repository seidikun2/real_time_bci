...
def get_decision(left, both, right, threshold=0.60):
    values = {LEFT_LABEL: float(left), BOTH_LABEL: float(both), RIGHT_LABEL: float(right)}
    label, value = max(values.items(), key=lambda kv: kv[1])
    if value >= threshold:
        return label
...
def update_plot(ax_pca, pca_point, pca_trace, bars, txt_decision, txt_phase,
    ...
    decision = get_decision(values[0], values[1], values[2], THRESHOLD)
